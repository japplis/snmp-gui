...
// TVE : Static entry class added below.
// used in : BrowserControl, to fix bug 325
...
public class BrowserLauncher {
    ...
    private static Object locateBrowser() {
        ...
        switch (jvm) {
            ...
            case WINDOWS_NT:
                ...
                break;
            case WINDOWS_9x:
                browser = "command.com"; // not needed
                break;
            case OTHER:
            default:
                browser = "netscape";
    ...
    public static void openURL(String url) throws IOException {
        ...
        if (url.toLowerCase().indexOf("javascript:")!=-1) {
            throw new IOException("Unsecure url " + url + " won't be executed");
        }
        switch (jvm) {
            case MRJ_2_0:
                Object aeDesc = null;
            ...
            case WINDOWS_NT:
                ...
                if (indexAmp!=-1 && (url.toLowerCase().startsWith("http:")
                      || url.toLowerCase().startsWith("file:")))
                    url = url.substring(0,5)+"\""+url.substring(5)+"\"";
                else if (indexAmp!=-1) break; // Security
            ...
            case WINDOWS_9x:
                //Runtime.getRuntime().exec(new String[] { (String) browser, FIRST_WINDOWS_PARAMETER,
                //                                                    SECOND_WINDOWS_PARAMETER, url });
                Runtime.getRuntime().exec(new String[] { SECOND_WINDOWS_PARAMETER, url });
                break;
            case OTHER:
                // Assume that we're on Unix and that Netscape is installed

                // replace quote in url by %27 and put the url between quotes
                // this is to avoid security pb. e.g. netscape 'test';rm -fr ~'
                url = replace("'",url);
                url = replace("\"",url);
                url = replace(" ",url);
                url = replace(";",url);
                ...
                /*try {
                    ...
                    if (exitCode != 0) {    // if Netscape was not open
                        System.out.println("not opened trying "+browser+" "+url);
    ...
    /**
     * Replace the caracter in the url by it's %ASCII code
     * @param car a String with the caracter to change (the String should be only
     * one caracter.
     * @the new url
     */
    public static String replace(String car, String url) {
        int indexQuote = url.indexOf(car);
        int asciiCode = (int)car.charAt(0);
        StringBuffer sbUrl = new StringBuffer(url);
        while (indexQuote!=-1) {
            sbUrl.deleteCharAt(indexQuote);
            sbUrl.insert(indexQuote,"%"+asciiCode);
            url = sbUrl.toString();
            indexQuote = url.indexOf(car,indexQuote);
        }
        return url;
    }
